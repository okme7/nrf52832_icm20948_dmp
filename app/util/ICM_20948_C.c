...
ICM_20948_Status_e ICM_20948_firmware_load(ICM_20948_Device_t *pdev)
{
  ...
  return (inv_icm20948_firmware_load(pdev, dmp3_image, sizeof(dmp3_image), DMP_LOAD_START));   //从ox90写入处理器内存
#else
...
ICM_20948_Status_e inv_icm20948_firmware_load(ICM_20948_Device_t *pdev, const unsigned char *data_start, unsigned short size_start, unsigned short load_addr)
{
  ...
  while (size > 0)
  {
    ...
    if ((memaddr & 0xff) + write_size > 0x100)   // <256
    {
      // Moved across a bank
...
#else
    result = inv_icm20948_write_mems(pdev, memaddr, write_size, (unsigned char *)data);    //写入失败！！！！
#endif
    if (result != ICM_20948_Stat_Ok)
      return result;
    data += write_size;
...
ICM_20948_Status_e inv_icm20948_write_mems(ICM_20948_Device_t *pdev, unsigned short reg, unsigned int length, const unsigned char *data)
{
  ...
  {
    lStartAddrSelected = (reg & 0xff);  //0x90
    ...
    /* Write data */
		  result = ICM_20948_execute_w(pdev, AGB0_REG_MEM_R_W, &dmp3[bytesWritten], thisLen);   //写入失败  0x7D  16

//    result = ICM_20948_execute_w(pdev, AGB0_REG_MEM_R_W, (uint8_t *)&data[bytesWritten], thisLen);   //写入失败  0x7D  16
//		result = ICM_20948_execute_w(pdev, AGB0_REG_MEM_START_ADDR, &lStartAddrSelected, 1);
...
{
  ICM_20948_Status_e result = ICM_20948_Stat_Ok;
  uint8_t fifoBytes[icm_20948_DMP_Maximum_Bytes]; // Interim storage for the FIFO data  14
...
//	printf("fifo_count %d\r\n",fifo_count);
  ...
  if (fifo_count < icm_20948_DMP_Header_Bytes) // Has a 2-byte header arrived?       2
