...
void Serif_t_initialize(ICM_20948_Serif_t* serif) {
    serif->user = serif;
}
...
ICM_20948_AGMT_t ICM_20948_getAGMT(void)
{
...
float ICM_20948_magX(void)
{
  return ICM_20948_getMagUT(agmt.mag.axes.x);
}

float ICM_20948_magY(void)
{
  return ICM_20948_getMagUT(agmt.mag.axes.y);
}

float ICM_20948_magZ(void)
{
  return ICM_20948_getMagUT(agmt.mag.axes.z);
}
...
float ICM_20948_accX(void)
{
  return ICM_20948_getAccMG(agmt.acc.axes.x);
}

float ICM_20948_accY(void)
{
  return ICM_20948_getAccMG(agmt.acc.axes.y);
}

float ICM_20948_accZ(void)
{
  return ICM_20948_getAccMG(agmt.acc.axes.z);
}
...
float ICM_20948_gyrX(void)
{
  return ICM_20948_getGyrDPS(agmt.gyr.axes.x);
}

float ICM_20948_gyrY(void)
{
  return ICM_20948_getGyrDPS(agmt.gyr.axes.y);
}

float ICM_20948_gyrZ(void)
{
  return ICM_20948_getGyrDPS(agmt.gyr.axes.z);
}
...
float ICM_20948_temp(void)
{
  return ICM_20948_getTempC(agmt.tmp.val);
}
...
const char *ICM_20948_statusString(ICM_20948_Status_e stat)
{
...
ICM_20948_Status_e ICM_20948_sleep_(bool on)
{
...
ICM_20948_Status_e ICM_20948_checkID(void)
{
  ...
  {
//    debugPrint(F("ICM_20948_checkID: ICM_20948_check_id returned: "));
...
uint8_t ICM_20948_getWhoAmI(void)
{
  uint8_t retval = 0x00;
...
bool ICM_20948_isConnected(void)
{
  status = ICM_20948_checkID();
  if (status == ICM_20948_Stat_Ok)
  {
    return true;
  }
//  debugPrint(F("ICM_20948_isConnected: checkID returned: "));
//  debugPrintStatus(status);
//  debugPrintln(F(""));
  return false;
}
...
ICM_20948_Status_e ICM_20948_setSampleMode(uint8_t sensor_id_bm, uint8_t lp_config_cycle_mode)
{
...
ICM_20948_Status_e ICM_20948_cfgIntLatch(bool latching)
{
  ICM_20948_INT_PIN_CFG_t reg;
  status = ICM_20948_int_pin_cfg(&_device, NULL, &reg); // read phase
  if (status != ICM_20948_Stat_Ok)
...
ICM_20948_Status_e ICM_20948_cfgIntAnyReadToClear(bool enabled)
{
  ICM_20948_INT_PIN_CFG_t reg;
  status = ICM_20948_int_pin_cfg(&_device, NULL, &reg); // read phase
  if (status != ICM_20948_Stat_Ok)
...
ICM_20948_Status_e ICM_20948_cfgFsyncIntMode(bool interrupt_mode)
{
  ICM_20948_INT_PIN_CFG_t reg;
  status = ICM_20948_int_pin_cfg(&_device, NULL, &reg); // read phase
  if (status != ICM_20948_Stat_Ok)
...
{
  return (ICM_20948_i2cControllerConfigurePeripheral(peripheral, addr, reg, len, Rw, enable, data_only, grp, swap, 0));
}
...
{
  return (ICM_20948_i2cControllerPeriph4Transaction(addr, reg, data, len, Rw, send_reg_addr));
}
...
uint8_t ICM_20948_i2cMasterSingleR(uint8_t addr, uint8_t reg)
{
  ...
  if (status != ICM_20948_Stat_Ok)
  {
//    debugPrint(F("ICM_20948_i2cMasterSingleR: ICM_20948_i2c_master_single_r returned: "));
//    debugPrintStatus(status);
//    debugPrintln(F(""));
  }
  return data;
...
//  {
//    printf("ICM_20948_startupDefault: checkID returned: ");
...
//  }
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
			printf("ICM_20948_startupDefault: swReset returned: ");
//    debugPrintStatus(retval);
//    debugPrintln(F(""));
    status = retval;
    return status;
  ...
  retval = ICM_20948_sleep_(false);
  if (retval != ICM_20948_Stat_Ok)
  {
    printf("ICM_20948_startupDefault: sleep returned: ");
//    debugPrintStatus(retval);
//    debugPrintln(F(""));
    status = retval;
    return status;
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
    printf("ICM_20948_startupDefault: lowPower returned: ");
//    debugPrintStatus(retval);
//    debugPrintln(F(""));
    status = retval;
    return status;
  ...
  retval = ICM_20948_startupMagnetometer(minimal); // Pass the minimal startup flag to startupMagnetometer
  if (retval != ICM_20948_Stat_Ok)
  {
    printf("ICM_20948_startupDefault: startupMagnetometer returned: ");
//    debugPrintStatus(retval);
//    debugPrintln(F(""));
    status = retval;
    return status;
  ...
  {
//    debugPrintln(F("ICM_20948_startupDefault: minimal startup complete!"));
    return status;
  }

  retval = ICM_20948_setSampleMode((ICM_20948_Internal_Acc | ICM_20948_Internal_Gyr), ICM_20948_Sample_Mode_Continuous); // options: ICM_20948_Sample_Mode_Continuous or ICM_20948_Sample_Mode_Cycled
  if (retval != ICM_20948_Stat_Ok)
  {
    printf("ICM_20948_startupDefault: setSampleMode returned: ");
//    debugPrintStatus(retval);
//    debugPrintln(F(""));
    status = retval;
    return status;
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
    printf("ICM_20948_startupDefault: setFullScale returned: ");
//    debugPrintStatus(retval);
//    debugPrintln(F(""));
    status = retval;
    return status;
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
    printf("ICM_20948_startupDefault: setDLPFcfg returned: ");
//    debugPrintStatus(retval);
//    debugPrintln(F(""));
    status = retval;
    return status;
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
    printf("ICM_20948_startupDefault: enableDLPF (Acc) returned: ");
//    debugPrintStatus(retval);
//    debugPrintln(F(""));
    status = retval;
    return status;
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
    printf("ICM_20948_startupDefault: enableDLPF (Gyr) returned: ");
//    debugPrintStatus(retval);
//    debugPrintln(F(""));
    status = retval;
    return status;
...
{
  uint8_t data = ICM_20948_i2cMasterSingleR(MAG_AK09916_I2C_ADDR, reg); // i2cMasterSingleR updates status too
  return data;
}

ICM_20948_Status_e ICM_20948_writeMag(AK09916_Reg_Addr_e reg, uint8_t *pdata)
{
  status = ICM_20948_i2cMasterSingleW(MAG_AK09916_I2C_ADDR, reg, *pdata);
  return status;
}
...
ICM_20948_Status_e ICM_20948_resetMag()
{
  ...
  status = ICM_20948_i2cMasterSingleW(MAG_AK09916_I2C_ADDR, AK09916_REG_CNTL3, SRST);
  return status;
}
...
ICM_20948_Status_e ICM_20948_setFIFOmode(bool snapshot)
{
...
//    debugPrint(F("ICM_20948_enableDMPSensor:  _enabled_Android_0: "));
...
//    debugPrint(F("ICM_20948_enableDMPSensorInt:  _enabled_Android_intr_0: "));
...
//    debugPrint(F("ICM_20948_setGyroSF:  pll: "));
...
//    debugPrint(F("ICM_20948_startupDMP: DMP is not available. Please check that you have uncommented line 29 (#define ICM_20948_USE_DMP) in ICM_20948_C.h..."));
    return ICM_20948_Stat_DMPNotSupported;
  }
...
//  mySmplrt.a = 19; // ODR is computed as follows: 1.125 kHz/(1+ACCEL_SMPLRT_DIV[11:0]). 19 = 56.25Hz. InvenSense Nucleo example uses 19 (0x13).
  ...
  result = ICM_20948_setDMPstartAddress(DMP_START_ADDRESS); if (result > worstResult) worstResult = result; // Defaults to DMP_START_ADDRESS  DMP程序启动地址  0x1000

  // Now load the DMP firmware
  result = ICM_20948_loadDMPFirmware(); if (result > worstResult) worstResult = result;    //下载dmp固件
  // Write the 2 byte Firmware Start Value to ICM PRGM_STRT_ADDRH/PRGM_STRT_ADDRL
  result = ICM_20948_setDMPstartAddress(DMP_START_ADDRESS); if (result > worstResult) worstResult = result; // Defaults to DMP_START_ADDRESS
  ...
  result = ICM_20948_setGyroSF(4, 3); if (result > worstResult) worstResult = result; // 19 = 55Hz (see above), 3 = 2000dps (see above)
...
//  // Set up the serif
//  I2C_serif.write = ICM_20948_write_I2C;
//  I2C_serif.read = ICM_20948_read_I2C;
////  I2C_Serif_t_initializeserif.user = (void *)this; // refer to yourself in the user field
//	Serif_t_initialize(&I2C_serif);
//  // Link the serif
//  _device._serif = &I2C_serif;
...
ICM_20948_Status_e ICM_20948_startupMagnetometer(bool minimal)
{
  ICM_20948_Status_e retval = ICM_20948_Stat_Ok;

  ICM_20948_i2cMasterPassthrough(false); //Do not connect the SDA/SCL pins to AUX_DA/AUX_CL
  ICM_20948_i2cMasterEnable(true);
  ...
  while (tries < MAX_MAGNETOMETER_STARTS)
  {
    ...
    retval = ICM_20948_magWhoIAm();
    if (retval == ICM_20948_Stat_Ok)
      break; //WIA matched!

    ICM_20948_i2cMasterReset(); //Otherwise, reset the master I2C and try again
...
//    debugPrintln(F("ICM_20948::startupMagnetometer: minimal startup complete!"));
  ...
  retval = ICM_20948_writeMag(AK09916_REG_CNTL2, (uint8_t *)&reg);
  if (retval != ICM_20948_Stat_Ok)
  {
...
//    debugPrintln(F(""));
  ...
  retval = ICM_20948_i2cControllerConfigurePeripheral(0, MAG_AK09916_I2C_ADDR, AK09916_REG_ST1, 9, true, true, false, false, false, 0);
  if (retval != ICM_20948_Stat_Ok)
  {
...
ICM_20948_Status_e ICM_20948_magWhoIAm(void)
{
  ...
  whoiam1 = ICM_20948_readMag(AK09916_REG_WIA1);
...
//    debugPrintln(F(""));
  ...
  whoiam2 = ICM_20948_readMag(AK09916_REG_WIA2);
  // readMag calls i2cMasterSingleR which calls ICM_20948_i2c_master_single_r
  // i2cMasterSingleR updates status so it is OK to set retval to status here
  retval = status;
  if (retval != ICM_20948_Stat_Ok)
...
//  _spi->endTransaction();
  ...
  // Set up the serif
  SPI_serif.write = ICM_20948_write_SPI;
  SPI_serif.read = ICM_20948_read_SPI;
//  SPI_serif.user = (void *)this; // refer to yourself in the user field
	Serif_t_initialize(&SPI_serif);
  // Link the serif
  _device._serif = &SPI_serif;
  ...
  // Perform default startup
  // Do a minimal startupDefault if using the DMP. User can always call startupDefault(false) manually if required.
//	printf("_dmp_firmware_available %d\r\n", _device._dmp_firmware_available);
  status = ICM_20948_startupDefault(_device._dmp_firmware_available);
  if (status != ICM_20948_Stat_Ok)
  {
...
{
	(void)user;
	write_multiple_spi(reg, data, len);
  return ICM_20948_Stat_Ok;
}
...
{
	(void)user;
	read_multiple_spi(reg, buff, len);
  return ICM_20948_Stat_Ok;
}
